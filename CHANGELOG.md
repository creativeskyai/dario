...
### Removed
- `--cli` / CLI backend mode — Removed. **Our implementation** was unreliable in practice: it had no tool use support, produced streaming artifacts, and handled multi-turn context by concatenating prior messages into the system prompt (which doesn't match how the model sees real conversation turns). A functional `claude --print`-based backend *is* achievable — see `openclaw-claude-bridge` for a working reference implementation — but it requires native-tool replacement via prompt-injected protocol, per-channel `--session-id`/`--resume` state, disk-backed session lifecycle, subprocess concurrency caps, and tight coupling to the calling client's semantics. That's roughly 10× the code of our old `--cli` mode and pulls dario's positioning away from "generic stateless API stealth via template replay" toward "subprocess multiplexer for a specific client." Not the direction we want to take, so the mode is gone. Direct API mode with template replay remains dario's single path.
